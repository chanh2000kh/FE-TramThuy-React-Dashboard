...
export default function ProductTramThuyEdit() {
  ...
  // -------------
  // Detail Product---------------
  const [detailProduct, setDetailProduct] = useState(EditorState.createEmpty());

  // Detail Product----------------
  const [listDescribeId, setListDescribeId] = useState([]);
  const [listDescribeCanChoose, setListDescribeCanChoose] = useState([]);
  const loadListDescribeCanChoose = (list) => {
    callApi(`api/Describe/getDescribe?limit=50&skip=1`, 'GET')
      .then((res) => {
        const newList = res.data.data.filter((data) => {
          let check = true;
          list.forEach((d) => {
            if (d._id === data._id) check = false;
          });
          return check;
        });
        setListDescribeCanChoose(newList);
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  // ------------------------------------
  const [productId, setproductId] = useState(GetURLParameter('id'));
  const loadData = () => {
    callApi(`api/product/getOneProduct?id=${productId}`, 'GET')
      .then((res) => {
        setName(res.data.data.name);
        setPrice(res.data.data.price);
        setNumberOfRemain(res.data.data.numberOfRemain);
        ...
        setproductTypeId(res.data.data.productTypeId);
        setDetailProduct(getEditorState(res.data.data.detailProduct));
        loadListDescribeCanChoose(res.data.data.listDescribeId);
  ...
  const saveEditProduct = () => {
    ...
    callApi(`api/product/updateProduct`, 'PUT', addProduct)
      .then((res) => {
  ...
  return (
    <Page title="Edit product">
      <Container>
        ...
        <Button onClick={saveEditProduct} variant="contained" style={{ background: '#32CD32' }}>
          Lưu
        </Button>
